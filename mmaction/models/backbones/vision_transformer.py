...
from timm.models.helpers import build_model_with_cfg, named_apply, adapt_input_conv, checkpoint_seq#, resolve_pretrained_cfg
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        print('self.qkv.weight.grad',self.qkv.weight.grad)
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):  # torch.Size([495, 3, 224, 224]) #torch.Size([4, 1569, 768])
