...
# from .transformer import build_transformer_layer_sequence
...
class TimeSformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.transformer_layers = build_transformer_layer_sequence(
            transformer_layers)
        '''
        self.transformer_layers = VisionTransformer()
    ...
    def forward(self, x):
        #x.shape torch.Size([4, 3, 8, 224, 224])
        import pdb
        """Defines the computation performed at every call."""
        # x [batch_size * num_frames, num_patches, embed_dims]
        batches = x.shape[0]
        x = self.patch_embed(x) #torch.Size([32, 196, 768])

        # x [batch_size * num_frames, num_patches + 1, embed_dims]
        cls_tokens = self.cls_token.expand(x.size(0), -1, -1) #torch.Size([32, 1, 768])
        x = torch.cat((cls_tokens, x), dim=1) #torch.Size([32, 197, 768])
        x = x + self.pos_embed          #torch.Size([32, 197, 768])
        x = self.drop_after_pos(x)      #torch.Size([32, 197, 768])

        # Add Time Embedding
        if self.attention_type != 'space_only':
            # x [batch_size, num_patches * num_frames + 1, embed_dims]
            cls_tokens = x[:batches, 0, :].unsqueeze(1) #torch.Size([4, 1, 768])
            x = rearrange(x[:, 1:, :], '(b t) p m -> (b p) t m', b=batches) #torch.Size([784, 8, 768])
            
            x = x + self.time_embed
            x = self.drop_after_time(x)#torch.Size([784, 8, 768])
            x = rearrange(x, '(b p) t m -> b (p t) m', b=batches)#torch.Size([4, 1568, 768])
            x = torch.cat((cls_tokens, x), dim=1)#torch.Size([4, 1569, 768])
        
        x = self.transformer_layers(x)#torch.Size([4, 1569, 768])
        
        # x = self.transformer_layers(x,None,None)#torch.Size([4, 1569, 768])
        ...
        x = self.norm(x)#torch.Size([4, 1569, 768])
